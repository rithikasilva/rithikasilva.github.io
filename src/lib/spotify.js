...
fetch(`${serviceHost}/get-now-playing`)
    ...
    if (data.hasOwnProperty("ERROR") || (data.hasOwnProperty("currently_playing_type") && data["currently_playing_type"] == "episode")) {
        ...
        songData = data;
        if (data.item.name.length > 18) {
            document.getElementById("player-song").innerHTML = data.item.name.substring(0,18) + `...`;
        } else {
            document.getElementById("player-song").innerHTML = data.item.name;
        }
        document.getElementById("player-artist").innerHTML = `by\n ` + data.item.artists[0].name;
